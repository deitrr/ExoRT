...
!============================================================================
  ...
  subroutine aerad_driver(ext_H2O, ext_CO2, ext_CH4, ext_H2, ext_N2, &
      ...
      ext_aldir, ext_asdif, ext_aldif, ext_srf_emiss, &
...
!
    ...
    DIRECTU(:,:) = 0.0
    DIREC(:,:) = 0.0
    TAUL(:,:) = 0.0      ;     TAULsol(:,:) = 0.0    ;     TAULir(:,:) = 0.0
    OPD(:,:) = 0.0       ;     OPDsol(:,:) = 0.0     ;    OPDir(:,:) = 0.0
    W0(:,:) = 0.0        ;     W0sol(:,:) = 0.0     ;    W0ir(:,:) = 0.0
    G0(:,:) = 0.0        ;     G0sol(:,:) = 0.0     ;    G0ir(:,:) = 0.0
    tau_gas(:,:) = 0.0
...
!       endif
    ...
    call calc_cldopd_h2o(ext_pint, cICE, cLIQ, REI, REL, cFRC, & 
    ...
    call rad_precalc(pmid/100.0, tmid, tint, swcut, tau_gas, tau_ray, &
                     ...
                     Y3, TAUL, TAULsol, TAULir, OPD, OPDsol, OPDir, &
                     PTEMP, PTEMPG, SLOPE, SOL, &
                     W0, W0sol, W0ir,  G0, G0sol, G0ir, &
                     EMIS, RSFXdir, RSFXdif)

    beamSolar = .true.  ! do solar calculation, all wavenlengths, two-stream quadrature
    ip_ibeg = sw_ipbeg
    ip_iend = sw_ipend
    call two_stream(TAULsol, W0sol, G0sol, EMIS, RSFXdir, RSFXdif, beamSolar, ip_ibeg, ip_iend, &
    ...
    call add_txrad(EM1sol, EM2sol, EL1sol, EL2sol, &
                   ...
                   TAULsol, OPDsol, PTEMP, PTEMPG, SLOPE, SOL, W0sol, G0sol, EMIS, RSFXdir, RSFXdif, cos_mu, sw_on, &
    ...
    ip_ibeg = lw_ipbeg
    ip_iend = lw_ipend
    call two_stream(TAULir, W0ir, G0ir, EMIS, RSFXdir,RSFXdif, beamSolar, ip_ibeg, ip_iend, &
    ...
    call add_txrad(EM1ir, EM2ir, EL1ir, EL2ir, &
                   ...
                   TAULir, OPDir, PTEMP, PTEMPG, SLOPE, SOL, W0ir, G0ir, EMIS, RSFXdir, RSFXdif, cos_mu, sw_on, &
                   ip_ibeg, ip_iend, beamSolar, CK1ir, CK2ir, CPBir, CMBir, B3ir, DIRECTir)    

    call refine_lwflux(CK1ir, CK2ir, Y3, AKir, GAMIir, B3ir, EE1ir, &
                       TAULir, PTEMP, PTEMPG, SLOPE, EMIS, RSFXdir, RSFXdif, cos_mu, DIRECTU, DIREC)
...
! 
    ...
    real(r8), intent(out), dimension(ntot_gpt,pverp) :: TAULsol
    real(r8), intent(out), dimension(ntot_gpt,pverp) :: TAULir   
    real(r8), intent(out), dimension(ntot_gpt,pverp) :: OPD       
    real(r8), intent(out), dimension(ntot_gpt,pverp) :: OPDsol
    real(r8), intent(out), dimension(ntot_gpt,pverp) :: OPDir       
    ...
    real(r8), intent(out), dimension(ntot_gpt,pverp) :: W0sol
    real(r8), intent(out), dimension(ntot_gpt,pverp) :: W0ir
    real(r8), intent(out), dimension(ntot_gpt,pverp) :: G0        
    real(r8), intent(out), dimension(ntot_gpt,pverp) :: G0sol
    real(r8), intent(out), dimension(ntot_gpt,pverp) :: G0ir        
...
! 
    ...
    real(r8), dimension(ntot_gpt,pverp) :: TAULabs
...
!     
    ...
    OPDsol(1:ntot_gpt,:) = 0.d0     ! Initialize necessary array portions in
    OPDir(1:ntot_gpt,:) = 0.d0     ! Initialize necessary array portions in
    TAUL(1:ntot_gpt,:) = 0.d0    !  anticipation of summing below
    TAULsol(1:ntot_gpt,:) = 0.d0    !  anticipation of summing below
    TAULir(1:ntot_gpt,:) = 0.d0    !  anticipation of summing below
    TAULabs(1:ntot_gpt,:) = 0.d0    !  anticipation of summing below
    ...
    do k=1,pverp     ! Loop over all layer BOUNDARIES  
      ...
      do iw=1,ntot_wavlnrng      ! Only necessary wavelength bands
        do ig=1,ngauss_pts(iw)
          ...
          do ip =1,2
!            if (do_exo_mcica) then   ! this option is in 1D only
 ...
 !           endif
          ...
          ! Toon et al. 1989 states     
          !"Hence the Eddington technique does not yield unit emissivity in this limit. Indeed, the Eddington technique can yield emissivity greater than unity even for the case when w0 is not equal to zero."  
          ...
          ! solar beam is delta-Eddington scaled
          TAULsol(it,k) = TAUL(it,k)
          W0sol(it,k) = W0(it,k)
          G0sol(it,k) = G0(it,k)

          ! infrared is delta-Eddington scaled
!          TAULir(it,k) = TAUL(it,k)
!          W0ir(it,k) = W0(it,k)
!          G0ir(it,k) = G0(it,k)

         
!          TAULsol(it,k) = taul_ig
!          W0sol(it,k) = w0_ig
!          G0sol(it,k) = g0_ig

          ! infrared is not delta scaled
          TAULir(it,k) = taul_ig
          W0ir(it,k) = w0_ig
          G0ir(it,k) = g0_ig
...
!write(*,*) it, k, w0_ig, g0_ig, TAULabs(it,k), TAULir(it,k), TAULsol(it,k)
          ...
          OPDsol(it,k) = OPDsol(it,k_1)+TAULsol(it,k)
         
          ! non delta scaled  
          OPDir(it,k) = OPDir(it,k_1)+TAULir(it,k)
         
!          write(*,*) "G0(it,k)", it, k, G0(it,k), G0sol(it,k), G0ir(it,k)
...
!          
    ...
    real(r8), dimension(ntot_gpt,pverp) :: ptemp_interface   
    real(r8) :: wl1
    real(r8) :: wl2
    real(r8) :: ft
    integer :: ip,iw,ig
...
!    endif
    ...
    do k=1,pverp      ! Loop over all layers
      do ip=lw_ipbeg,lw_ipend   ! Loop over all longwave gauss points
        ...
        ! Determine interface planck function
        ! Slope derived from interface planck
        it = int(tint(k)*tpft_finc)
        ft = dble(tint(k)*tpft_finc-real(it))
        ...
        if(TAUL(ip,k) > SMALLd) then
          ...
          !SLOPE(ip,k) = (ptemp_interface(ip,k)-ptemp_interface(ip,k_1)) / (TAUL(ip,k))  ! 
        else
          SLOPE(ip,k) = 0.d0          
        endif
